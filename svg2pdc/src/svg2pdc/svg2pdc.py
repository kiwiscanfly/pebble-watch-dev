...
#
# Vendored from pebble-examples/cards-example (tools/svg2pdc.py) and modernized
# for Python 3: print() calls, binary file output, list(element) instead of
# getchildren(), ''.join(filter(...)), and a relative import for the color
# routines. The PDC binary format and conversion logic are otherwise unchanged.
'''
...
def convert_color(rgb, a, truncate=True):
    from .pebble_image_routines import pebble_nearest_color_to_pebble_palette, \
        pebble_truncate_color_to_pebble_palette, rgba32_triplet_to_argb8
        ...
        if tag == 'layer' or tag == 'g':
            ...
            if tag == 'g':
              ...
              if group_stroke_width:
                group_stroke_width = int(float(''.join(filter(lambda x: x in '0123456789.', group_stroke_width))))
